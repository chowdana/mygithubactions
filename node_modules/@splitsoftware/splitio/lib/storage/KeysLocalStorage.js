...
var KeyBuilderForLocalStorage = /*#__PURE__*/function (_KeyBuilder) {
  ...
  _proto.buildSegmentNameKey = function buildSegmentNameKey(segmentName) {
    return this.settings.storage.prefix + "." + (0, _factory.matching)(this.settings.core.key) + ".segment." + segmentName;
  };

  _proto.extractSegmentName = function extractSegmentName(builtSegmentKeyName) {
    var prefix = this.settings.storage.prefix + "." + (0, _factory.matching)(this.settings.core.key) + ".segment.";
  ...
  _proto.buildOldSegmentNameKey = function buildOldSegmentNameKey(segmentName) {
    return (0, _factory.matching)(this.settings.core.key) + "." + this.settings.storage.prefix + ".segment." + segmentName;
  } // @BREAKING: The key used to start with the matching key instead of the prefix, this was changed on version 10.17.3
  ;

  _proto.extractOldSegmentKey = function extractOldSegmentKey(maybeOldKey) {
    var prefix = (0, _factory.matching)(this.settings.core.key) + "." + this.settings.storage.prefix + ".segment.";
    if ((0, _lang.startsWith)(maybeOldKey, prefix)) return maybeOldKey.substr(prefix.length);
  };
